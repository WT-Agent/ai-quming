  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">起名类型</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">风格基调</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">品牌命名与国学起名避坑指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 品牌商标与宝宝/个人起名专家');
...
const metricsList = [
  {
    "key": "phoneticHarmonyFlow",
    "label": "音律声调和谐度"
  },
  {
    "key": "culturalConnotationDepth",
    "label": "文化意蕴内涵度"
  },
  {
    "key": "brandDomainAvailability",
    "label": "品牌传播易记性"
  },
  {
    "key": "characterStrokeBalance",
    "label": "字形笔画美感度"
  },
  {
    "key": "uniquenessOriginality",
    "label": "独特独创防重分"
  }
];
const guideRules = [
  {
    "title": "平仄音律和谐",
    "advice": "名字声调注意平仄相间，读起来朗朗上口、抑扬顿挫，避免生僻字与多音字",
    "avoid": "切忌使用容易产生谐音闹剧的字组合"
  },
  {
    "title": "寓意深刻不俗套",
    "advice": "从经典诗词典故或自然万物中汲取灵感，赋予名字深刻的人格或品牌灵魂",
    "avoid": "避免使用过于大众化、同质化严重的俗套字"
  },
  {
    "title": "商标防重与传播",
    "advice": "品牌起名前必须在商标网查询类别防重，确保名字易记、易写且域名可注册",
    "avoid": "严禁模仿大牌搞山寨谐音侵权名称"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[QUMING_SCORES\][\s\S]*?\[\/QUMING_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[QUMING_SCORES\](.*?)\[\/QUMING_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('quming_history_records');
...
const saveHistory = () => {
  localStorage.setItem('quming_history_records', JSON.stringify(historyList.value));
};
